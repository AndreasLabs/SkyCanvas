...
class SpiralMission(Mission):
    ...
    async def run(self, context: QuadContext, waypoints: WaypointSystem):
        ...
        logging.info(
            "Quad // Waiting for takeoff altitude %.2fm +/- %.2fm",
            takeoff_alt,
            altitude_tolerance,
        )
        async for position in context.mav_system.telemetry.position():
            if abs(position.relative_altitude_m - takeoff_alt) <= altitude_tolerance:
                logging.info(
                    "Quad // Takeoff altitude reached at %.2fm",
                    position.relative_altitude_m,
                )
                break

        logging.info("Quad // Holding takeoff altitude for 3s")
